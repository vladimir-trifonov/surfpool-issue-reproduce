...
export async function executeSkill(blockhash: string): Promise<string> {
  ...
  // Strategy: Use larger amount and different direction for better rewards
  const swapYtoX = tokenXMint.equals(NATIVE_MINT) ? false : true; // Swap X to Y instead
  const inMint = swapYtoX ? tokenYMint : tokenXMint;
  const outMint = swapYtoX ? tokenXMint : tokenYMint;
  const amountIn = 100_000_000n; // 0.1 SOL or 100 tokens - much larger amount
  ...
  // Optimize compute budget for maximum efficiency
  ...
  // Prepare Meteora swap instruction with all bin arrays for maximum liquidity access
  ...
  // Use all available bin arrays for maximum liquidity
  const binArrayPubkeys = Array.isArray(METEORA.binArrays) ? METEORA.binArrays : [];
  const remaining = binArrayPubkeys.map((pk) => ({
  ...
  const data = getSwap2InstructionDataEncoder().encode({
    amountIn,
    minAmountOut: 1n, // Set minimum to ensure some output
