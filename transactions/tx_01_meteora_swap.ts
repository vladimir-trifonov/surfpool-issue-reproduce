...
export async function executeSkill(blockhash: string): Promise<string> {
  ...
  // Strategy: Swap SOL to token for potential rewards
  const swapYtoX = tokenYMint.equals(NATIVE_MINT) ? true : false;
  const inMint = swapYtoX ? tokenYMint : tokenXMint;
  const outMint = swapYtoX ? tokenXMint : tokenYMint;
  const amountIn = 50_000_000n; // 0.05 SOL - larger amount for better rewards potential
  ...
  // Prepare Meteora swap instruction
